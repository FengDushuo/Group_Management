 ...
 * @param {*} param 
 */
const getWxUsers = async (param) => {
    const { current = 0, pageSize = 10, username, startTime, endTime } = param
    let sql = `select SQL_CALC_FOUND_ROWS ${usersColumns.join(',')} from wx_users`
    const res = await exec(sql)
    console.log(res)
...
const getWxUser = async (param) => {
    ...
    let sql = `select ${usersColumns.join(',')} from wx_users where `
    if (id) {
        sql += `id=${id}`
    } 
    const res = await exec(sql)
 ...
 * 获取所有用户
 */
const getAllWxUsers = async () => {
    const sql = `select id,open_id,nickname,avatar,birth,phone,city,province,gender from wx_users order by id desc`
    const res = await exec(sql)
    return new SuccessModel({
        data: res
    })
